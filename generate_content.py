...
def research_topic(client: anthropic.Anthropic, history: list[str], mode: str) -> str:
    ...
    if mode == "proven":
        task = (
            ...
            "something real people use and discuss. Anchor this pick to a HEAD-TERM tool "
            f"people already search for and hashtag -- e.g. {head_terms}, or a similarly "
            "well-known name. Do not avoid it for being well-known: this account has no "
            "followers yet, so Search and hashtags (which only surface existing query "
            "volume) are the only discovery paths that don't require an audience already, "
            "and there is no search volume for an obscure pick.\n\n"
            "The differentiator must be the ANGLE, not the tool's obscurity: a specific "
            "underused feature, an unusual use case, or a direct comparison ('the Notion AI "
            "feature nobody uses', 'X vs Grammarly'). Find the angle on that well-known tool "
            "that this audience likely hasn't seen before."
        )
    else:
        task = (
            ...
            "If you pick an established product -- including a head-term name like "
            f"{head_terms} -- the angle must be something it shipped in that window, and "
            "that new thing must be what the post is about. A head-term tool's recent "
            "feature is a good pick: it carries existing search/hashtag demand AND a fresh "
            "angle.\n\n"
